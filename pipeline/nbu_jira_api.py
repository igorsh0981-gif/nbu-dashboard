...
FIELDS = [
    ...
    'customfield_10102',  # Epic Link (ключ эпика — подтверждено для этой Jira)
]

DONE_STATUSES = {
    'done', 'готово к релизу', 'protocol approved', 'closed', 'resolved',
    'ready for release', 'rft'
}
INPROG_STATUSES = {
    'in progress', 'code review', 'customer agreement', 'в работе'
}
TESTING_STATUSES = {
    'testing', 'ready for testing', 'тестирование'
}
ANALYTICS_STATUSES = {
...
# ─── НОРМАЛИЗАЦИЯ ВЕРСИЙ ─────────────────────────────────────────────────────
def fix_version(v):
    """2.6.2000 → 2.6.0; 3.0 → 3.0.0 (нормализация до X.Y.Z)"""
    v = v.strip()
    v = re.sub(r'\.\d{4}$', '.0', v)
    # Если версия вида "X.Y" (ровно 2 сегмента, оба числа, не A_-релиз) — дописываем .0
...
for issue in all_issues:
    ...
    if status_name.lower() in DONE_STATUSES:
        status_cat = 'done'
    elif status_name.lower() in TESTING_STATUSES:
        status_cat = 'testing'
    elif status_name.lower() in ANALYTICS_STATUSES:
        status_cat = 'analytics'
    ...
    rows.append({
        ...
        'Ссылка на эпик': str(f.get('customfield_10102') or '').strip(),
...
# ─── STATUS HISTORY — формируем df, запись делает run_all.py ─────────────────
df_history = pd.DataFrame(history_rows)
print(f"✅ status_history сформирован: {len(df_history)} записей (запись через run_all.py)")
