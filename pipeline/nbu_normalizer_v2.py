...
# ─── ЧТЕНИЕ ДАННЫХ ───────────────────────────────────────────────────────────
# Используем df_csv из Jira API если уже загружен, иначе читаем CSV файл
if 'df_csv' in dir() and hasattr(df_csv, '__len__') and len(df_csv) > 100:
    ...
    df = df_csv[df_csv['Тип задачи'] != 'Epic'].copy()
else:
    print(f"Читаем CSV файл: {CSV_FILE}")
    df = pd.read_csv(CSV_FILE, low_memory=False)
    df = df[df['Тип задачи'] != 'Epic'].copy()
...
for _, row in df.iterrows():
    ...
    # Определяем epic_name: если тип Epic — это сам эпик, иначе берём из parent
    is_epic = (issue_type == 'Epic')
    epic_link  = key if is_epic else (parent if parent and parent != 'nan' else '')
    epic_name  = summary if is_epic else ''
...
headers = [
    ...
    'assignee', 'priority', 'release',
    'components', 'created', 'updated', 'due_date',
    'parent_id', 'labels', 'epic_link', 'epic_name', 'updated_at',
]

# Статистика
no_rel  = sum(1 for r in rows_out if not r[7])
with_rel = len(rows_out) - no_rel
# Второй проход: проставляем epic_name для задач по parent_id
# Строим словарь: key эпика → epic_name (summary)
epic_names = {}
for r in rows_out:
    if r[2] == 'Epic':  # type == Epic
        epic_names[r[0]] = r[1]  # key → title

# Проставляем epic_name для всех задач чей parent_id = ключ эпика
IDX_PARENT  = headers.index('parent_id')
IDX_EPIC_L  = headers.index('epic_link')
IDX_EPIC_N  = headers.index('epic_name')
for r in rows_out:
    if r[2] != 'Epic' and r[IDX_PARENT]:
        ep_name = epic_names.get(r[IDX_PARENT], '')
        r[IDX_EPIC_N] = ep_name
        if not r[IDX_EPIC_L]:
            r[IDX_EPIC_L] = r[IDX_PARENT]
