...
for _, row in df.iterrows():
    ...
    # Epic Link — кастомное поле Jira (для Story/Task)
    epic_link_raw = str(row.get('Epic Link', '') or row.get('Epic Link 2', '') or '').strip()
    if epic_link_raw == 'nan': epic_link_raw = ''

    parent_id  = parent if parent and parent != 'nan' else ''

    # Приоритет: customfield Epic Link > parent если parent это эпик
