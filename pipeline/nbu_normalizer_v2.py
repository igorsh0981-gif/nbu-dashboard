...
for _, row in df.iterrows():
    ...
    # Определяем epic через parent_id → EPIC_NAMES
    parent_id  = parent if parent and parent != 'nan' else ''
    epic_link  = parent_id if parent_id and parent_id in EPIC_NAMES else ''
    epic_name  = EPIC_NAMES.get(parent_id, '') if parent_id else ''
...
print(f"С эпиком:     {sum(1 for r in rows_out if r[headers.index('epic_name')])}")
